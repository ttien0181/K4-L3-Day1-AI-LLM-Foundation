...
def call_openai(
    ...
    response = client.chat.completions.create(
        ...
        # 3. Bấm giờ ngay sau khi nhận phản hồi từ API
    end_time = time.perf_counter()
    latency_seconds = end_time - start_time

    # 4. Trích xuất văn bản phản hồi và trả về đúng thứ tự tuple
...
def call_openai_mini(
    ...
    """
    return call_openai(prompt=prompt, temperature=temperature, top_p=top_p, max_tokens=max_tokens, model=OPENAI_MINI_MODEL)
...
def format_comparison_table(results: list[dict]) -> str:
    ...
    """
    def _cut(text: str, length: int = 40) -> str:
        text = str(text)
        return text if len(text) <= length else text[: length - 1] + "…"

    header = (
        f"{'Prompt':<45} | {'GPT-4o Response':<41} | "
        f"{'Mini Response':<41} | {'GPT-4o Lat':>10} | {'Mini Lat':>10}"
    )
    ...
    for r in results:
        lines.append(
            f"{_cut(r.get('prompt', ''), 44):<45} | "
            f"{_cut(r.get('gpt4o_response', ''), 40):<41} | "
            f"{_cut(r.get('mini_response', ''), 40):<41} | "
            f"{r.get('gpt4o_latency', 0):>10.3f} | "
            f"{r.get('mini_latency', 0):>10.3f}"
        )
